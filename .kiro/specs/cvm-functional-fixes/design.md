...
validator split). Deploying those behavior changes unconditionally would cause a
**chain split** between upgraded and non-upgraded nodes. The design therefore
gates all consensus-behavior-changing fixes behind a **new activation flag**
(`DEPLOYMENT_CVM_FIX` / `cvmFunctionalFixHeight`) so that the corrected
validation rules take effect at a coordinated height on every node
simultaneously. Non-consensus fixes (RPC output, local metrics, Sybil scoring
that does not gate block validity) ship unconditionally because they cannot
split the chain.
...
location, (3) gate consensus-behavior changes behind the activation flag, and
(4) verify with unit, property-based, and functional tests that the bug is fixed
for buggy inputs and that non-buggy inputs behave identically.
...
- **Consensus-critical fix**: A fix that changes which blocks/transactions are
  accepted. Must be activation-gated to avoid chain splits.
- **Activation gate**: A block-height / version-bits deployment
  (`DEPLOYMENT_CVM_FIX`, `cvmFunctionalFixHeight`) below which nodes retain the
  legacy behavior and at/above which they enforce the corrected behavior.
...
Implementation section. The consensus-critical specialization additionally
carries the activation dimension:

```
FUNCTION isConsensusBugCondition(input, height, params)
  RETURN isBugCondition(input)
         AND pathIsConsensusCritical(input)          // 1.1,1.2,1.16-1.23,1.60,1.61
         AND IsCVMFixActive(height, params) == true  // corrected rules enforced
END FUNCTION
```
...
unaffected. For consensus-critical fixes, all inputs at heights **below** the
`cvmFunctionalFixHeight` activation must also be completely unaffected — legacy
behavior is preserved historically so the existing chain remains valid.
...
behavior` (each mapping to one or more Expected Behavior clauses 2.x).
Properties 21–22 are preservation properties that assert
`¬isBugCondition ⇒ F'(x) = F(x)` (mapping to Unchanged Behavior clauses 3.x).
Consensus-critical properties are additionally quantified over the activation
height (`cvmFunctionalFixHeight`).

Property 1: Bug Condition — Per-block subsidy limit enforced

_For any_ block whose CVM/EVM transactions carry gas subsidies, evaluated at a
height where the CVM fix is active, the fixed accounting SHALL accumulate the
actual per-transaction subsidies and reject the block when the total exceeds the
per-block subsidy maximum.
...
_For any_ coinbase evaluated when validator participation data is available and
the fix is active, `CheckCoinbaseValidatorPayments` SHALL enforce the 70/30
validator payment split.
...
**Validates: Requirements 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.7, 3.8, 3.9, 3.10, 3.11, 3.13, 3.14, 3.15, 3.16, 3.18, 3.19, 3.20, 3.21, 3.22, 3.23, 3.24**

Property 22: Preservation — Historical chain validity under activation gate

_For any_ consensus-critical fix, at any block height below
`cvmFunctionalFixHeight`, the fixed node SHALL apply exactly the legacy validation
behavior, so the existing chain history and cross-node agreement are preserved
and no chain split occurs at upgrade time.

**Validates: Requirements 3.12, 3.17**
...
### Activation-Gating Strategy (applies to all consensus-critical workstreams)

**Problem:** Changing which blocks are valid (subsidy limits, gas cost,
address derivation, signature enforcement, state persistence/rollback, coinbase
split) without coordination causes upgraded nodes to reject blocks that
non-upgraded nodes accept (and vice versa) — a chain split.

**Approach:**
1. Add a new deployment `DEPLOYMENT_CVM_FIX` to `Consensus::DeploymentPos`
   (`src/consensus/params.h`) and a companion height field
   `cvmFunctionalFixHeight` (mirroring the existing `cvmActivationHeight` /
   `DEPLOYMENT_CVM_EVM` pattern) set per network in `chainparams.cpp`.
2. Add a helper `bool IsCVMFixActive(int height, const Consensus::Params&)`
   (alongside `IsCVMEVMEnabled` in `cvm/activation.{h,cpp}`).
3. In every consensus-critical fix site, branch on `IsCVMFixActive(...)`:
   below the gate run the **exact legacy code path** (preserved verbatim), at/above
   the gate run the corrected path. This makes Property 22 hold by construction.
4. Consensus-critical clauses gated this way: **1.1, 1.2, 1.16, 1.17, 1.18,
   1.19, 1.20, 1.21, 1.22, 1.23, 1.60, 1.61.** (1.61 is gated because durable
   persistence changes the committed state set that other nodes must reproduce.)
5. Non-consensus clauses ship **unconditionally** — they change local/RPC/scoring
   behavior only and cannot split the chain.

**Coordination note:** The two `ProcessCVMBlock` (`cvmtx.cpp`) and primary
(`blockprocessor.cpp`) execution paths must be reconciled so that, above the
gate, contract address derivation, execution, value/block-hash context, and
state commit are consistent between them (3.12). Divergence between the two paths
above the gate would itself be a consensus fault.
...
Concrete changes (all gated on `IsCVMFixActive`):
...
**Consensus-critical:** 1.23 (gated); 1.24–1.26 change execution results and are
also gated because they affect block-processing outcomes above the gate.
...
- Note: fee-model changes that affect the accepted fee/subsidy split for CVM
  transactions interact with consensus; where they change block validity they
  must respect the same activation gate as Workstream 1. Eligibility that matched
  under both old and new checks is preserved (3.16); free-gas zero-fee path and
  the previous fixed-rate split are preserved for matching inputs (3.8, 3.1).
...
- **`tx_priority.cpp` / `blockprocessor.cpp` address extraction** (pseudo-address
  ...
  (2.56). This is consensus-adjacent where it feeds block processing — where it
  changes block outcomes it follows the Workstream-1 activation gate.
...
the bug is fixed for buggy inputs (fix checking) and that non-buggy inputs — and,
for consensus fixes, all sub-activation inputs — behave identically (preservation
checking). Consensus fixes are additionally exercised with the activation gate
both off (legacy) and on (corrected).
...
// consensus-critical:
FOR ALL (input, height) WHERE isConsensusBugCondition(input, height, params) DO
  ASSERT correctedValidation(input, height) == specifiedBehavior
END FOR
```
...
produces the same result as the original; and for consensus fixes, all
sub-activation heights reproduce legacy behavior exactly.
...
FOR ALL (input, height) WHERE height < cvmFunctionalFixHeight DO
  ASSERT fixedNode.validate(input, height) = legacyNode.validate(input, height)
END FOR
```
...
6. Blocks below `cvmFunctionalFixHeight` validate identically (3.12, 3.17, 3.2).
...
- **Preservation property (Property 21):** for random standard/WoT transactions,
  fixed and original validation results are identical.
- **Preservation property (Property 22):** for random inputs at random heights
  below the gate, fixed-node and legacy-node validation results are identical.
...
- Functional (`test/functional/feature_cvm.py` and new cases): deploy → call a
  ...
  1.17, 1.60, 1.61, 3.12).
- Activation transition: build a regtest chain spanning `cvmFunctionalFixHeight`;
  assert blocks below the gate follow legacy rules and blocks at/above follow
  corrected rules, with no reorg/split at the boundary (Property 22).
