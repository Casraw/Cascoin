...
// Cascoin: MinotaurX+Labyrinth1.2: Diff adjustment for pow algos (post-MinotaurX activation)
// Modified LWMA-3
...
unsigned int GetNextWorkRequiredLWMA(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params, const POW_TYPE powType) {
    ...
    while (blocksFound < N) {
        ...
        // Cascoin: MinotaurX+Labyrinth1.2: Check for old version blocks only relevant if MinotaurX is NOT enabled.
        ...
        if (blockPreviousTimestamp->GetBlockHeader().IsLabyrinthMined(params) || effectivePrevBlockType != powType) {
...
unsigned int DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    // Cascoin: Labyrinth 1.1: Skip over Labyrinthmined blocks at tip
    if (IsLabyrinth11Enabled(pindexLast, params)) {
        while (pindexLast->GetBlockHeader().IsLabyrinthMined(params)) {
            //LogPrintf("DarkGravityWave: Skipping labyrinthmined block at %i\n", pindex->nHeight);
    ...
    for (unsigned int nCountBlocks = 1; nCountBlocks <= nPastBlocks; nCountBlocks++) {
        // Cascoin: Labyrinth: Skip over Labyrinthmined blocks; we only want to consider PoW blocks
        while (pindex->GetBlockHeader().IsLabyrinthMined(params)) {
            //LogPrintf("DarkGravityWave: Skipping labyrinthmined block at %i\n", pindex->nHeight);
...
bool CheckProofOfWork(uint256 hash, unsigned int nBits, const Consensus::Params& params)
{
    ...
    // Cascoin: MinotaurX+Labyrinth1.2: Use highest pow limit for limit check
...
unsigned int GetNextLabyrinth11WorkRequired(const CBlockIndex* pindexLast, const Consensus::Params& params) {
    const arith_uint256 bnPowLimit = UintToArith256(params.powLimitLabyrinth);

    arith_uint256 mouseHashTarget = 0;
    int labyrinthBlockCount = 0;
    int totalBlockCount = 0;

    // Step back till we have found 24 labyrinth blocks, or we ran out...
    while (labyrinthBlockCount < params.labyrinthDifficultyWindow && pindexLast->pprev && pindexLast->nHeight >= params.minLabyrinthCheckBlock) {
        if (pindexLast->GetBlockHeader().IsLabyrinthMined(params)) {
    ...
    if (labyrinthBlockCount == 0) {          // Should only happen when chain is starting
        LogPrint(BCLog::LABYRINTH, "GetNextLabyrinth11WorkRequired: No previous labyrinth blocks found.\n");
        return bnPowLimit.GetCompact();
    }
...
// Cascoin: MinotaurX+Labyrinth1.2: Reset Labyrinth difficulty after MinotaurX enable
unsigned int GetNextLabyrinth12WorkRequired(const CBlockIndex* pindexLast, const Consensus::Params& params) {
    const arith_uint256 bnPowLimit = UintToArith256(params.powLimitLabyrinth);
    ...
    // Step back till we have found 24 labyrinth blocks, or we ran out...
    while (labyrinthBlockCount < params.labyrinthDifficultyWindow && pindexLast->pprev && IsMinotaurXEnabled(pindexLast, params)) {        
        if (pindexLast->GetBlockHeader().IsLabyrinthMined(params)) {
            mouseHashTarget += arith_uint256().SetCompact(pindexLast->nBits);
            labyrinthBlockCount++;
        }
    ...
    if (labyrinthBlockCount < 24) {
        // Always return the easiest difficulty during blockchain startup
        // This helps bootstrap The Labyrinth mining system
        LogPrint(BCLog::LABYRINTH, "GetNextLabyrinth12WorkRequired: Insufficient labyrinth blocks - using easiest target for bootstrapping.\n");
        return bnPowLimit.GetCompact();
    }
...
unsigned int GetNextLabyrinthWorkRequired(const CBlockIndex* pindexLast, const Consensus::Params& params) {
    // Cascoin: MinotaurX+Labyrinth1.2
    if (IsMinotaurXEnabled(pindexLast, params))
        return GetNextLabyrinth12WorkRequired(pindexLast, params);
    // Cascoin: Labyrinth 1.1: Use SMA diff adjust
    if (IsLabyrinth11Enabled(pindexLast, params))
        return GetNextLabyrinth11WorkRequired(pindexLast, params);

    const arith_uint256 bnPowLimit = UintToArith256(params.powLimitLabyrinth);
    const arith_uint256 bnImpossible = 0;
    arith_uint256 mouseHashTarget;

    //LogPrintf("GetNextLabyrinthWorkRequired: Height     = %i\n", pindexLast->nHeight);
    ...
    while (true) {
        if (!pindexLast->pprev || pindexLast->nHeight < params.minLabyrinthCheckBlock) {   // Ran out of blocks without finding a Labyrinth block? Return min target
            LogPrint(BCLog::LABYRINTH, "GetNextLabyrinthWorkRequired: No labyrinthmined blocks found in history\n");
            //LogPrintf("GetNextLabyrinthWorkRequired: This target= %s\n", bnPowLimit.ToString());
            return bnPowLimit.GetCompact();
        }

        block = pindexLast->GetBlockHeader();
        if (block.IsLabyrinthMined(params)) {  // Found the last Labyrinth block; pick up its mouse hash target
    ...
    //LogPrintf("GetNextLabyrinthWorkRequired: powBlocks  = %i\n", numPowBlocks);
    if (numPowBlocks == 0)
        return bnImpossible.GetCompact();

    //LogPrintf("GetNextLabyrinthWorkRequired: Last target= %s\n", mouseHashTarget.ToString());

	// Apply EMA
	int interval = params.labyrinthTargetAdjustAggression / params.labyrinthBlockSpacingTarget;
...
bool GetNetworkLabyrinthInfo(int& immatureMice, int& immatureBCTs, int& matureMice, int& matureBCTs, CAmount& potentialLifespanRewards, const Consensus::Params& consensusParams, bool recalcGraph) {
    ...
    // Cascoin: MinotaurX+Labyrinth1.2: Get correct labyrinth block reward
    ...
    for (int i = 0; i < totalMouseLifespan; i++) {
        ...
        if (!pindexPrev->GetBlockHeader().IsLabyrinthMined(consensusParams)) {                          // Don't check Labyrinthmined blocks (no BCTs will be found in them)
            ...
            if (block.vtx.size() > 0) {
                for(const auto& tx : block.vtx) {
                    ...
                    if (tx->IsBCT(consensusParams, scriptPubKeyBCF, &mouseFeePaid)) {                 // If it's a BCT, total its mice
                        ...
                        if (recalcGraph) {
                            /*
                            int mouseStart = blockHeight + consensusParams.mouseGestationBlocks;
                            int mouseStop = mouseStart + consensusParams.mouseLifespanBlocks;
                            mouseStart -= tipHeight;
                            mouseStop -= tipHeight;
                            for (int j = mouseStart; j < mouseStop; j++) {
                            ...
                            int mouseDiesBlock = mouseMaturesBlock + consensusParams.mouseLifespanBlocks;
                            for (int j = mouseBornBlock; j < mouseDiesBlock; j++) {
...
bool CheckLabyrinthProof(const CBlock* pblock, const Consensus::Params& consensusParams) {
    bool verbose = LogAcceptCategory(BCLog::LABYRINTH);

    if (verbose)
        LogPrintf("********************* Labyrinth: CheckLabyrinthProof *********************\n");
    ...
    if (!pindexPrev) {
        LogPrint(BCLog::LABYRINTH, "CheckLabyrinthProof: Couldn't get previous block's CBlockIndex!\n");
        return false;
    }
    if (verbose)
        LogPrintf("CheckLabyrinthProof: nHeight             = %i\n", blockHeight);

    // Check labyrinth is enabled on network
    if (!IsLabyrinthEnabled(pindexPrev, consensusParams)) {
        LogPrint(BCLog::LABYRINTH, "CheckLabyrinthProof: Can't accept a Labyrinth block; Labyrinth is not yet enabled on the network.\n");
        return false;
    }

    // Cascoin: Labyrinth 1.1: Check that there aren't too many consecutive Labyrinth blocks
    if (IsLabyrinth11Enabled(pindexPrev, consensusParams)) {
        int labyrinthBlocksAtTip = 0;
        CBlockIndex* pindexTemp = pindexPrev;
        while (pindexTemp->GetBlockHeader().IsLabyrinthMined(consensusParams)) {
            assert(pindexTemp->pprev);
            pindexTemp = pindexTemp->pprev;
            labyrinthBlocksAtTip++;
        }
        if (labyrinthBlocksAtTip >= consensusParams.maxConsecutiveLabyrinthBlocks) {
            LogPrint(BCLog::LABYRINTH, "CheckLabyrinthProof: Too many Labyrinth blocks without a POW block.\n");
            return false;
        }
    } else {
        if (pindexPrev->GetBlockHeader().IsLabyrinthMined(consensusParams)) {
            LogPrint(BCLog::LABYRINTH, "CheckLabyrinthProof: Labyrinth block must follow a POW block.\n");
            return false;
        }
    }
    ...
    if (pblock->vtx.size() > 1)
        for (unsigned int i=1; i < pblock->vtx.size(); i++)
            if (pblock->vtx[i]->IsBCT(consensusParams, scriptPubKeyBCF)) {
                LogPrintf("CheckLabyrinthProof: Labyrinthmined block contains BCTs!\n");
                return false;                
            }
    
    // Coinbase tx must be valid
    CTransactionRef txCoinbase = pblock->vtx[0];
    //LogPrintf("CheckLabyrinthProof: Got coinbase tx: %s\n", txCoinbase->ToString());
    if (!txCoinbase->IsCoinBase()) {
        LogPrintf("CheckLabyrinthProof: Coinbase tx isn't valid!\n");
        return false;
    }

    // Must have exactly 2 or 3 outputs
    if (txCoinbase->vout.size() < 2 || txCoinbase->vout.size() > 3) {
        LogPrintf("CheckLabyrinthProof: Didn't expect %i vouts!\n", txCoinbase->vout.size());
        return false;
    }

    // vout[0] must be long enough to contain all encodings
    if (txCoinbase->vout[0].scriptPubKey.size() < 144) {
        LogPrintf("CheckLabyrinthProof: vout[0].scriptPubKey isn't long enough to contain labyrinth proof encodings\n");
        return false;
    }
    ...
    if (txCoinbase->vout[0].scriptPubKey[0] != OP_RETURN || txCoinbase->vout[0].scriptPubKey[1] != OP_MOUSE) {
        LogPrintf("CheckLabyrinthProof: vout[0].scriptPubKey doesn't start OP_RETURN OP_MOUSE\n");
        return false;
    }
    ...
    if (verbose)
        LogPrintf("CheckLabyrinthProof: mouseNonce            = %i\n", mouseNonce);
    ...
    if (verbose)
        LogPrintf("CheckLabyrinthProof: bctTxId             = %s\n", txidStr);

    // Check mouse hash against target
    std::string deterministicRandString = GetDeterministicRandString(pindexPrev);
    if (verbose)
        LogPrintf("CheckLabyrinthProof: detRandString       = %s\n", deterministicRandString);
    arith_uint256 mouseHashTarget;
    mouseHashTarget.SetCompact(GetNextLabyrinthWorkRequired(pindexPrev, consensusParams));
    if (verbose)
        LogPrintf("CheckLabyrinthProof: mouseHashTarget       = %s\n", mouseHashTarget.ToString());
    
    // Cascoin: MinotaurX+Labyrinth1.2: Use the correct inner Labyrinth hash
    if (!IsMinotaurXEnabled(pindexPrev, consensusParams)) {
        ...
        if (verbose)
            LogPrintf("CheckLabyrinthProof: mouseHash             = %s\n", mouseHash.GetHex());
        if (mouseHash >= mouseHashTarget) {
            LogPrintf("CheckLabyrinthProof: Mouse does not meet hash target!\n");
            return false;
        }
    } else {
        ...
        if (verbose)
            LogPrintf("CheckLabyrinth12Proof: mouseHash           = %s\n", mouseHash.GetHex());
        if (mouseHash >= mouseHashTarget) {
            LogPrintf("CheckLabyrinth12Proof: Mouse does not meet hash target!\n");
            return false;
        }
    }
    ...
    if (!ExtractDestination(txCoinbase->vout[1].scriptPubKey, cheeseDestination)) {
        LogPrintf("CheckLabyrinthProof: Couldn't extract cheese address\n");
        return false;
    }
    if (!IsValidDestination(cheeseDestination)) {
        LogPrintf("CheckLabyrinthProof: Cheese address is invalid\n");
        return false;
    }
    if (verbose)
        LogPrintf("CheckLabyrinthProof: cheeseAddress        = %s\n", EncodeDestination(cheeseDestination));

    // Verify the message sig
    const CKeyID *keyID = boost::get<CKeyID>(&cheeseDestination);
    if (!keyID) {
        LogPrintf("CheckLabyrinthProof: Can't get pubkey for cheese address\n");
        return false;
    }
    ...
    if (!pubkey.RecoverCompact(mhash, messageSig)) {
        LogPrintf("CheckLabyrinthProof: Couldn't recover pubkey from hash\n");
        return false;
    }
    if (pubkey.GetID() != *keyID) {
        LogPrint(BCLog::LABYRINTH, "CheckLabyrinthProof: Signature mismatch! GetID() = %s, *keyID = %s\n", pubkey.GetID().ToString(), (*keyID).ToString());
        return false;
    }
    ...
    bool bctWasMinotaurXEnabled;    // Cascoin: MinotaurX+Labyrinth1.2: Track whether Labyrinth 1.2 was enabled at BCT creation time
    {
        LOCK(cs_main);
        ...
        if (pcoinsTip && pcoinsTip->GetCoin(outMouseCreation, coin)) {        // First try the UTXO set (this pathway will hit on incoming blocks)
            if (verbose)
                LogPrintf("CheckLabyrinthProof: Using UTXO set for outMouseCreation\n");
            bctValue = coin.out.nValue;
            bctScriptPubKey = coin.out.scriptPubKey;
            bctFoundHeight = coin.nHeight;
            bctWasMinotaurXEnabled = IsMinotaurXEnabled(chainActive[bctFoundHeight], consensusParams);  // Cascoin: MinotaurX+Labyrinth1.2: Track whether Labyrinth 1.2 was enabled at BCT creation time
            ...
            if (verbose)
                LogPrintf("! CheckLabyrinthProof: Warn: Using deep drill for outMouseCreation\n");
            if (!GetTxByHashAndHeight(uint256S(txidStr), bctClaimedHeight, bct, foundAt, pindexPrev, consensusParams)) {
                LogPrintf("CheckLabyrinthProof: Couldn't locate indicated BCT\n");
                return false;
            }
            deepDrill = true;
            ...
            bctWasMinotaurXEnabled = IsMinotaurXEnabled(&foundAt, consensusParams); // Cascoin: MinotaurX+Labyrinth1.2: Track whether Labyrinth 1.2 was enabled at BCT creation time
        }
        ...
        if (communityContrib) {
            ...
            if(bct == nullptr) {                                                                // If we dont have a ref to the BCT
                if (pcoinsTip && pcoinsTip->GetCoin(outCommFund, coin)) {                       // First try UTXO set
                    if (verbose)
                        LogPrintf("CheckLabyrinthProof: Using UTXO set for outCommFund\n");
                    if (coin.out.scriptPubKey != scriptPubKeyCF) {                              // If we find it, validate the scriptPubKey and store amount
                        LogPrintf("CheckLabyrinthProof: Community contrib was indicated but not found\n");
                        return false;
                    }
                    donationAmount = coin.out.nValue;
                } else {                                                                        // Fallback if we couldn't use UTXO set
                    if (verbose)
                        LogPrintf("! CheckLabyrinthProof: Warn: Using deep drill for outCommFund\n");
                    if (!GetTxByHashAndHeight(uint256S(txidStr), bctClaimedHeight, bct, foundAt, pindexPrev, consensusParams)) {
            ...
            if (donationAmount != expectedDonationAmount) {
                LogPrintf("CheckLabyrinthProof: BCT pays community fund incorrect amount %i (expected %i)\n", donationAmount, expectedDonationAmount);
                return false;
            }
    ...
    if (bctFoundHeight != bctClaimedHeight) {
        LogPrintf("CheckLabyrinthProof: Claimed BCT height of %i conflicts with found height of %i\n", bctClaimedHeight, bctFoundHeight);
        return false;
    }
    ...
    if (bctDepth < consensusParams.mouseGestationBlocks) {
        LogPrintf("CheckLabyrinthProof: Indicated BCT is immature.\n");
        return false;
    }
    if (bctDepth > consensusParams.mouseGestationBlocks + consensusParams.mouseLifespanBlocks) {
        LogPrintf("CheckLabyrinthProof: Indicated BCT is too old.\n");
        return false;
    }
    ...
    if (!CScript::IsBCTScript(bctScriptPubKey, scriptPubKeyBCF, &scriptPubKeyCheese)) {
        LogPrintf("CheckLabyrinthProof: Indicated utxo is not a valid BCT script\n");
        return false;
    }

    CTxDestination cheeseDestinationBCT;
    if (!ExtractDestination(scriptPubKeyCheese, cheeseDestinationBCT)) {
        LogPrintf("CheckLabyrinthProof: Couldn't extract cheese address from BCT UTXO\n");
        return false;
    }

    // Check BCT's cheese address actually matches the claimed cheese address
    if (cheeseDestination != cheeseDestinationBCT) {
        LogPrintf("CheckLabyrinthProof: BCT's cheese address does not match claimed cheese address!\n");
        return false;
    }
    ...
    if (bctValue < consensusParams.minMouseCost) {
        LogPrintf("CheckLabyrinthProof: BCT fee is less than the minimum possible mouse cost\n");
        return false;
    }
    if (bctValue < mouseCost) {
        LogPrintf("CheckLabyrinthProof: BCT fee is less than the cost for a single mouse\n");
        return false;
    }
    unsigned int mouseCount = bctValue / mouseCost;
    if (verbose) {
        LogPrintf("CheckLabyrinthProof: bctValue            = %i\n", bctValue);
        LogPrintf("CheckLabyrinthProof: mouseCost             = %i\n", mouseCost);
        LogPrintf("CheckLabyrinthProof: mouseCount            = %i\n", mouseCount);
    }
    
    // Check enough mice were bought to include claimed mouseNonce
    if (mouseNonce >= mouseCount) {
        LogPrintf("CheckLabyrinthProof: BCT did not create enough mice for claimed nonce!\n");
        return false;
    }

    if (verbose)
        LogPrintf("CheckLabyrinthProof: Pass at %i%s\n", blockHeight, deepDrill ? " (used deepdrill)" : "");
