...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nLabyrinthCheckThreads"))
        settings.setValue("nLabyrinthCheckThreads", (qint64)DEFAULT_LABYRINTH_THREADS);
    ...
    if (!settings.contains("nLabyrinthCheckDelay"))
        settings.setValue("nLabyrinthCheckDelay", (qint64)DEFAULT_LABYRINTH_CHECK_DELAY);
    ...
    if (!settings.contains("fLabyrinthCheckEarlyOut"))
        settings.setValue("fLabyrinthCheckEarlyOut", DEFAULT_LABYRINTH_EARLY_OUT);
    ...
    if (!settings.contains("fLabyrinthContribCF"))
        settings.setValue("fLabyrinthContribCF", DEFAULT_LABYRINTH_CONTRIB_CF);
    fLabyrinthContribCF = settings.value("fLabyrinthContribCF").toBool();
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        // Cascoin: MinotaurX+Labyrinth1.2
        case LabyrinthContribCF:
            return settings.value("fLabyrinthContribCF");
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        // Cascoin: MinotaurX+Labyrinth1.2
        case LabyrinthContribCF:
            fLabyrinthContribCF = value.toBool();
            if (settings.value("fLabyrinthContribCF") != value)
                settings.setValue("fLabyrinthContribCF", fLabyrinthContribCF);
            break;
