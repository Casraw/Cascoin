...
// Cascoin: MinotaurX+Labyrinth1.2: Accept POW_TYPE arg
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, const CScript* labyrinthProofScript, const POW_TYPE powType)
{
    ...
    if (labyrinthProofScript && !IsLabyrinthEnabled(pindexPrev, chainparams.GetConsensus()))
    ...
    // Cascoin: MinotaurX+Labyrinth1.2: Refuse to attempt to create a non-sha256 block before activation
    ...
    // Cascoin: MinotaurX+Labyrinth1.2: If MinotaurX is enabled, and we're not creating a Labyrinth block, encode desired pow type.
    if (!labyrinthProofScript && IsMinotaurXEnabled(pindexPrev, chainparams.GetConsensus())) {
    ...
    // Cascoin: Don't include BCTs in labyrinthmined blocks
    if (labyrinthProofScript)
        fIncludeBCTs = false;
    ...
    if (labyrinthProofScript) {
        ...
        coinbaseTx.vout[0].scriptPubKey = *labyrinthProofScript;
        coinbaseTx.vout[0].nValue = 0;

        // vout[1]: Cheese :)
        coinbaseTx.vout[1].scriptPubKey = scriptPubKeyIn;

        // Cascoin: MinotaurX+Labyrinth1.2: Labyrinth rewards are 150% of base block reward
        ...
        // Cascoin: MinotaurX+Labyrinth1.2: Pow rewards are 50% of base block reward
    ...
    if (labyrinthProofScript)
        pblock->nBits = GetNextLabyrinthWorkRequired(pindexPrev, chainparams.GetConsensus());
    else {
        // Cascoin: MinotaurX+Labyrinth1.2: If MinotaurX is enabled, handle nBits with pow-specific diff algo
        if (IsMinotaurXEnabled(pindexPrev, chainparams.GetConsensus()))
    ...
    // Cascoin: Labyrinth: Set nonce marker for labyrinthmined blocks
    pblock->nNonce = labyrinthProofScript ? chainparams.GetConsensus().labyrinthNonceMarker : 0;
    pblocktemplate->vTxSigOpsCost[0] = WITNESS_SCALE_FACTOR * GetLegacySigOpCount(*pblock->vtx[0]);

    // Cascoin: Diagnostic log for SHA256 on new chain
    if (!labyrinthProofScript && powType == POW_TYPE_SHA256 && IsMinotaurXEnabled(pindexPrev, chainparams.GetConsensus())) {
        ...
        while (tempIndex && count < chainparams.GetConsensus().lwmaAveragingWindow * 2) { // Check a reasonable depth
            if (!tempIndex->GetBlockHeader().IsLabyrinthMined(chainparams.GetConsensus()) && tempIndex->GetBlockHeader().GetPoWType() == POW_TYPE_SHA256) {
...
void MouseKeeper(const CChainParams& chainparams) {
    ...
    try {
        while (true) {
            ...
            int sleepTime = std::max((int64_t) 1, gArgs.GetArg("-labyrinthcheckdelay", DEFAULT_LABYRINTH_CHECK_DELAY));
...
// Cascoin: MinotaurX+Labyrinth1.2: Thread to check a single mouse bin
...
bool BusyMice(const Consensus::Params& consensusParams, int height) {
    ...
    if (IsLabyrinth11Enabled(pindexPrev, consensusParams)) {
        ...
        // Check previous block wasn't labyrinthmined
    ...
    int threadCount = gArgs.GetArg("-labyrinthcheckthreads", DEFAULT_LABYRINTH_THREADS);
    if (threadCount == -2)
    ...
    int binID = 0;
    bool minotaurXEnabled = IsMinotaurXEnabled(pindexPrev, consensusParams);    // Cascoin: MinotaurX+Labyrinth1.2: Check if minotaurX enabled
    while (mouseBinIterator != mouseBins.end()) {
        ...
        // Cascoin: MinotaurX+Labyrinth1.2: Use correct inner hash
        if (!minotaurXEnabled)
    ...
    bool useEarlyAbortThread = gArgs.GetBoolArg("-labyrinthearlyout", DEFAULT_LABYRINTH_EARLY_OUT);
    ...
    labyrinthProofScript << OP_RETURN << OP_MOUSE << mouseNonceVec << bctHeightVec << communityContribFlag << txidVec << messageProofVec;
    ...
    std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(Params()).CreateNewBlock(cheeseScript, true, &labyrinthProofScript));
    if (!pblocktemplate.get()) {
