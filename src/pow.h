...
unsigned int GetNextLabyrinthWorkRequired(const CBlockIndex* pindexLast, const Consensus::Params& params);                       // Cascoin: Labyrinth: Get the current Mouse Hash Target
unsigned int GetNextWorkRequiredLWMA(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params, const POW_TYPE powType); // Cascoin: MinotaurX+Labyrinth1.2: LWMA difficulty adjustment for all pow types
bool CheckLabyrinthProof(const CBlock* pblock, const Consensus::Params& params);                                                 // Cascoin: Labyrinth: Check The Labyrinth proof for given block
