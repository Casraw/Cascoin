...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-labyrinthcheckdelay=<ms>", strprintf(_("Time between Labyrinth checks in ms. This should be left at default unless performance degradation is observed (default: %u)"), DEFAULT_LABYRINTH_CHECK_DELAY));
    strUsage += HelpMessageOpt("-labyrinthcheckthreads=<threads>", strprintf(_("Number of threads to use when checking mice, -1 for all available cores, or -2 for one less than all available cores (default: %u)"), DEFAULT_LABYRINTH_THREADS));
    strUsage += HelpMessageOpt("-labyrinthearlyabort", strprintf(_("Abort Labyrinth checking as quickly as possible when a new block comes in. This should be left enabled unless performance degradation is observed. (default: %u)"), DEFAULT_LABYRINTH_EARLY_OUT));

    // Cascoin: MinotaurX+Labyrinth1.2: Allow switching of default pow algo via conf / command line, for miners that can't easily adjust their getblocktemplate calls
